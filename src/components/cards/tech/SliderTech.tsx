...
export function SliderTech({ logos, direction = "left" }: LogoSliderProps) {
  ...
  return (
    <div className="overflow-hidden m-0">
      <ul className={`flex ${directionLogo} gap-12`} style={{ minWidth: `${logosLoop.length * 10}rem` }}>
        {logosLoop.map((logo, i) => {
          return (
            <li key={i}
              className="group relative flex items-center justify-center w-16 h-16 md-tablet:w-20 
                md-tablet:h-20 table-lg:w-24 table-lg:h-24 transition-all duration-300"
            >
              {/* Glow animado */}
              <div className="absolute inset-0 rounded-full opacity-0 group-hover:opacity-80
                  blur-lg bg-colorHover2 transition-opacity duration-500"
              />
