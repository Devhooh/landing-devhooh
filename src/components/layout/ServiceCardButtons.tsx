...
export default function ServiceCardButtons({ slug }: ServiceCardButtonsProps) {
  ...
  const cardLoadingOverlay = isPending ? (
    <div className="absolute inset-0 bg-white/10 backdrop-blur-sm z-20 flex items-center justify-center rounded-2xl">
    ...
    <div className="mb-12 px-5 flex flex-col gap-4">
      ...
      <Link 
        ...
        className={`
          ...
          font-semibold overflow-hidden hover:bg-colorPrimario5/70 transition-colors flex items-center justify-center
      ...
      <Link
        ...
        className={`
          ...
          hover:bg-colorPrimario5/30 transition-colors transform delay-100 flex items-center justify-center
          ${isPending ? "opacity-50 cursor-not-allowed pointer-events-none" : ""}`}
      >
        Ver más detalles
      </Link>
