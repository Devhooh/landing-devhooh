...
export default function HeroSection({
  ...
  layout = "left",
  backgroundImageSrc,
  backgroundColor = "bg-white", // fondo neutro 60%
}: HeroProps) {
  const isLeftLayout = layout === "left";
  const isRightLayout = layout === "right";
  const isCenterLayout = layout === "center";
  ...
  const contentOrder = isLeftLayout ? "table-lg:order-1" : "table-lg:order-2";
  const imageOrder = isLeftLayout ? "table-lg:order-2" : "table-lg:order-1";
  const contentAlignment = isCenterLayout
    ? "text-center"
    : isRightLayout
    ? "text-center table-lg:text-right"
    : "text-center table-lg:text-left";
  const contentMargin = isCenterLayout
    ? "mx-auto"
    : isRightLayout
    ? "mx-auto table-lg:ml-auto table-lg:mr-0"
    : "mx-auto table-lg:mr-auto table-lg:ml-0";
  const buttonAlignment = isRightLayout
    ? "mx-auto table-lg:ml-auto table-lg:mr-0"
    : "mx-auto table-lg:mr-auto table-lg:ml-0";
    ...
    >
      <div className="max-w-7xl mx-auto flex flex-col table-lg:flex-row items-center gap-8 md-tablet:gap-16">
        ...
        <div className={`flex-1 ${contentAlignment} ${isCenterLayout ? "w-full" : contentOrder}`}>
          ...
          <p className={`text-lg md-tablet:text-xl text-gray-700 my-10 max-w-2xl ${contentMargin}`}>
            {subtitle}
          </p>
          ...
          {cta && ctaLink && (
            <Link href={ctaLink}>
              <button
                className={`px-8 py-4 text-lg md:text-xl font-semibold bg-fuchsia-500 text-white rounded-xl shadow-md hover:bg-fuchsia-600 hover:scale-105 transform transition-all duration-300 ${buttonAlignment}`}
              >
                {cta}
              </button>
            </Link>
        ...
        {/* Imagen del hero */}
        {imageSrc && !isCenterLayout && (
          <div className={`flex-1 hidden table-lg:flex justify-center ${imageOrder}`}>
